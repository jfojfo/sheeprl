...
from sheeprl.algos.dreamer_ppo.utils import choose_latent_state, generate_attention_mask, apply_lower_triangle_mask
...
class WorldModel(nn.Module):
    def __init__(self, cfg: Dict[str, Any],
        ...
        transformer = MyTransformerEncoderLayer(
            ...
            max_seq_len=cfg.algo.per_rank_sequence_length + cfg.algo.world_model.transformer.lookback_seq_len,
        )
    ...
    def dynamic(self, embedded_obs: Tensor, actions: Tensor, is_first: Tensor) \
        ...
        lb = self.cfg.algo.world_model.transformer.lookback_seq_len
        posterior_logits, posterior_stochastic_state = self._representation(embedded_obs)

        attn_output = self._attn_output(posterior_stochastic_state[:-1], actions[:-1], is_first[:-1], lb)
        attn_output = self._correct_is_first(attn_output, is_first[lb:])

        prior_logits, prior_stochastic_state = self._transition(attn_output)
        return posterior_logits[lb:], posterior_stochastic_state[lb:], prior_logits, prior_stochastic_state, attn_output
    ...
    def _attn_output(self, stochastic_state: Tensor, actions: Tensor, is_first: Tensor, lb: int) -> Tensor:
        embed_seq = self._embed_seq(stochastic_state, actions)
        query, kv = embed_seq[lb-1:], embed_seq

        attn_mask, _ = generate_attention_mask(is_first.squeeze(-1).transpose(0, 1))
        attn_mask = attn_mask[:, lb-1:]
        attn_mask = apply_lower_triangle_mask(attn_mask)
        ...
        attn_output, _, _ = self.transformer(query, kv, attn_mask=attn_mask)
        return attn_output

    def _embed_seq(self, stochastic_state: Tensor, actions: Tensor) -> Tensor:
        posterior = stochastic_state.view(*stochastic_state.shape[:-2], -1)
        embed_seq = torch.cat([self.embed_state(posterior), self.embed_action(actions)], dim=-1)
        return embed_seq

    def _correct_is_first(self, attn_output: Tensor, is_first: Tensor):
...
class PlayerDV3(nn.Module):
    ...
    def get_actions(
        ...
        cfg = self.cfg
        lb = cfg.algo.world_model.transformer.lookback_seq_len
        ...
        seq_obs = {k: torch.cat([self.seq_obs[k], obs[k]], dim=0) for k in obs}
        seq_is_first = torch.cat([self.seq_is_first, is_first], dim=0)

        seq_embedded_obs = self.world_model.encoder(seq_obs)
        ...
        if self.seq_action.shape[0] == 0:
            ...
            attn_output = self.world_model._attn_output(stochastic_state[:-1], self.seq_action, seq_is_first[:-1], stochastic_state[:-1].shape[0])
            attn_output = self.world_model._correct_is_first(attn_output, seq_is_first[-1:])

        latent_state = self.world_model.latent(stochastic_state[-1:], attn_output)

        actions, _ = self.actor(latent_state, greedy, mask)
        self.seq_action = torch.cat([self.seq_action, torch.cat(actions, dim=-1)], dim=0)[-lb:]
        self.seq_obs = {k: seq_obs[k][-lb:] for k in seq_obs}
        self.seq_is_first = seq_is_first[-lb:]
...
def train(
    ...
    continues_targets = 1 - data["terminated"][lb:]

    observation_loss = -sum([po[k].log_prob(batch_obs[k][lb:]) for k in po.keys()])
    reward_loss = -pr.log_prob(data["rewards"][lb:])
    ...
    true_continue = (1 - data["terminated"][lb:]).flatten().reshape(1, -1, 1)
