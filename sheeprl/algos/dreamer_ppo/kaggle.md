...
!cd /kaggle && git clone -b my-exp https://github.com/jfojfo/sheeprl
...
```
!cd /kaggle/sheeprl && MPLBACKEND=TkAgg /kaggle/miniconda/envs/py310/bin/python sheeprl.py exp=dreamer_ppo env=atari env.id=PongNoFrameskip-v4 env.num_envs=4 fabric.accelerator=gpu checkpoint.every=20000 metric.log_every=2000 buffer.checkpoint=False exp_name='${algo.name}_${env.id}'
```