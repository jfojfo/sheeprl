...
class WorldModel(nn.Module):
    def __init__(self, cfg: Dict[str, Any],
        ...
        history_query = nn.Parameter(nn.init.xavier_uniform_(torch.empty(1, 1, world_model_cfg.transformer.embed_dim)))
    ...
    def _attn_output(self, stochastic_state: Tensor, actions: Tensor, is_first: Tensor) -> Tuple[Tensor, Optional[Tuple[Tensor, Tensor]]]:
        ...
        seq_len, batch_size, embed_dim = embed_seq.shape
        query = self.history_query.expand(seq_len, batch_size, embed_dim)
        attn_output, _, kv_cache = self.transformer(query, embed_seq, attn_mask=attn_mask)
    ...
    def imagination(self, stochastic_state: Tensor, action: Tensor, kv_cache: tuple[Tensor, Tensor]) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        ...
        query = self.history_query.expand(1, *embed.shape[1:])
        attn_output, _, kv_cache = self.transformer(query, embed, kv_cache=kv_cache)
