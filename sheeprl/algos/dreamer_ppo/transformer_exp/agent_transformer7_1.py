...
def train(
    ...
    posterior_logits, posterior_stochastic_state, prior_logits, attn_output = (
        posterior_logits[1:], posterior_stochastic_state[1:], prior_logits[1:], attn_output[1:])
    ...
    continues_targets = 1 - data["terminated"][1:]

    observation_loss = -sum([po[k].log_prob(batch_obs[k][1:]) for k in po.keys()])
    reward_loss = -pr.log_prob(data["rewards"][1:])
    ...
    true_continue = (1 - data["terminated"][1:]).flatten().reshape(1, -1, 1)
