...
class WorldModel(nn.Module):
    ...
    def dynamic(self, embedded_obs: Tensor, actions: Tensor, is_first: Tensor) \
        ...
        attn_output = self._get_history_state(posterior_stochastic_state, actions, is_first)
    ...
    def _get_history_state(self, stochastic_state: Tensor, actions: Tensor, is_first: Tensor) -> Tensor:
        posterior = stochastic_state.view(*stochastic_state.shape[:-2], -1)
        embed_seq = self._embed_and_shift(posterior, actions, is_first)
        attn_output = self._attn_output(embed_seq, is_first)
        return attn_output

    def _embed_and_shift(self, posterior: Tensor, actions: Tensor, is_first: Tensor) -> Tensor:
        embed_seq = torch.cat([self.embed_state(posterior), self.embed_action(actions)], dim=-1)
        # 右移一个后，所有历史(states,actions)经attn output后与posterior当前位置对齐
        embed_seq = torch.cat((self.get_initial_states(embed_seq[:1].shape[:2]), embed_seq[:-1]), dim=0)
    ...
    def _attn_output(self, embed_seq: Tensor, is_first: Tensor) -> Tensor:
