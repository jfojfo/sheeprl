...
class MySelfAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = query.view(tgt_len, batch_size * self.num_heads, -1).transpose(0, 1)

        seq_len, batch_size, embed_dim = key.size()
        key = key.view(seq_len, batch_size * self.num_heads, -1).transpose(0, 1)

        seq_len, batch_size, embed_dim = value.size()
        value = value.view(seq_len, batch_size * self.num_heads, -1).transpose(0, 1)
        ...
        output = output.transpose(0, 1).contiguous().view(tgt_len, batch_size, embed_dim)
        return output
...
class WorldModel(nn.Module):
    def __init__(self, cfg: Dict[str, Any], actions_dim: Sequence[int], obs_space: gymnasium.spaces.Dict, is_continuous: bool):
        ...
        assert encoder.output_dim % 3 == 0
        attn_model = MySelfAttention(
    ...
    def _representation(self, embedded_obs: Tensor, is_first: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        q, k, v = embedded_obs.chunk(3, dim=-1)
        q, k, v = q.contiguous(), k.contiguous(), v.contiguous()
        feat = self.attn_model(q, k, v, attn_mask=attn_mask)
        logits = self._uniform_mix(feat)
