...
class WorldModel(nn.Module):
    def __init__(self, cfg: Dict[str, Any],
        ...
        latent_state_size = stoch_state_size
    ...
    def dynamic(self, embedded_obs: Tensor, actions: Tensor, is_first: Tensor) \
        ...
        next_logits, next_stochastic_state = self._transition(attn_output)
    ...
    def imagination(self, prior: Tensor, action: Tensor, kv_cache: tuple[Tensor, Tensor]) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        embed_seq = torch.cat([self.embed_state(prior), self.embed_action(action)], dim=-1)
        attn_output, kv_cache, _ = self.transformer(embed_seq, embed_seq, kv_cache=kv_cache)
...
class PlayerDV3(nn.Module):
    ...
    def get_actions(
        ...
        logits, stochastic_state = self.world_model._representation(seq_embedded_obs, self.seq_is_first)
        posterior = stochastic_state.view(*stochastic_state.shape[:-2], -1)
        # latent_state = torch.cat([posterior[-1:], attn_output[-1:]], dim=-1)
        latent_state = posterior[-1:]
...
def train(
    ...
    kv_cache = (
        torch.empty(0, imagined_prior.shape[1], cfg.algo.world_model.transformer.embed_dim).to(device),
        torch.empty(0, imagined_prior.shape[1], cfg.algo.world_model.transformer.embed_dim).to(device),
    )
    ...
    for i in range(1, cfg.algo.horizon + 1):
        ...
        imagined_latent_state = imagined_prior
